...
**企业 MCP + 工作流双路线（规划）**：MCP 工具开发 × NL 工作流应用 — 见 [`docs/MCP-WORKFLOW-PLATFORM-PLAN.md`](docs/MCP-WORKFLOW-PLATFORM-PLAN.md)。
...
**完整使用说明：** [`docs/WORKFLOW.md`](docs/WORKFLOW.md)（DSL 例子 + REST API + Agent 触发 + subflow + Dry-Run + 不变量）。**文档状态索引（计划 vs 代码）：** [`docs/DOC-STATUS.md`](docs/DOC-STATUS.md)。
...
env override：`PCA_MCP_ENABLED` / `PCA_MCP_HEARTBEAT_INTERVAL` / `PCA_MCP_INVOKE_TIMEOUT` / `PCA_MCP_LIST_TOOLS_TIMEOUT`。compose 同时跑 `mock-mcp:8083`（`internal/mcp/mockserver`，P0 三工具 `echo` / `fetch_status` / `record_event`），E2E 步 63 走 register → tools/list → invoke；P0 工作流见 `deploy/compose/examples/e2e-mock-chain.yaml`。
