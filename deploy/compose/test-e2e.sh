...
echo "[1/28] starting compose ..."
docker compose up -d --build >/dev/null
sleep 20

echo "[2/28] inserting demo user via psql ..."
...
echo "[8/28] verify 404 after destroy ..."
...
echo "[9/28] chat completion (non-stream) via mock-provider ..."
...
echo "[10/28] chat completion (stream) via mock-provider ..."
...
echo "[11/28] embeddings via mock-provider ..."
...
echo "[12/28] verify model_usage rows ..."
...
echo "[14/28] fs.write + fs.read round-trip ..."
...
echo "[16/28] llm.chat + tool_invocations ..."
...
echo "[17/28] agent.run direct final ..."
...
echo "[18/28] agent.run with tool_call chain ..."
...
echo "[20/28] GET /sessions + GET /sessions/:id/messages ..."
...
echo "[21/28] WS round-trip via docker websocat ..."
...
echo "[22/28] POST /memories x2 (different types) ..."
...
echo "[23/28] GET /memories?type=preference&tag=go filter ..."
...
echo "[24/28] memory.save via tool -> memory.search via tool round-trip ..."
...
echo "[25/28] DELETE /memories/{id} -> GET 404 ..."
...
# ---- Slice 8: Web UI ----
echo "[26/28] GET / returns SPA shell html ..."
...
echo "[27/28] GET /login (SPA fallback) returns the same shell ..."
