...
echo "[1/48] starting compose ..."
docker compose up -d --build >/dev/null
sleep 20

echo "[2/48] inserting demo user via psql ..."
...
echo "[8/48] verify 404 after destroy ..."
...
echo "[9/48] chat completion (non-stream) via mock-provider ..."
...
echo "[10/48] chat completion (stream) via mock-provider ..."
...
echo "[11/48] embeddings via mock-provider ..."
...
echo "[12/48] verify model_usage rows ..."
...
echo "[14/48] fs.write + fs.read round-trip ..."
...
echo "[16/48] llm.chat + tool_invocations ..."
...
echo "[17/48] agent.run direct final ..."
...
echo "[18/48] agent.run with tool_call chain ..."
...
echo "[20/48] GET /sessions + GET /sessions/:id/messages ..."
...
echo "[21/48] WS round-trip via docker websocat ..."
...
echo "[21b/48] DELETE /sessions/:id archives session and releases sandbox ..."
curl -fsS -X DELETE "http://localhost:8080/sessions/$SID" \
  -H "Authorization: Bearer $TOK" >/dev/null

echo "[22/48] POST /memories x2 (different types) ..."
...
echo "[23/48] GET /memories?type=preference&tag=go filter ..."
...
echo "[24/48] memory.save via tool -> memory.search via tool round-trip ..."
...
echo "[25/48] DELETE /memories/{id} -> GET 404 ..."
...
# ---- Slice 8: Web UI ----
echo "[26/48] GET / returns SPA shell html ..."
...
echo "[27/48] GET /login (SPA fallback) returns the same shell ..."
HTML2=$(curl -fsS http://localhost:8080/login)
echo "$HTML2" | grep -q 'id="root"' || { echo "spa fallback failed for /login"; exit 1; }

echo "[28/48] API not shadowed by SPA fallback: GET /sessions returns JSON ..."
...
# ---- Slice 9: Audit ----
echo "[29/48] GET /audit (admin) returns the access log ..."
...
echo "[30/48] GET /audit?action=auth.login finds login event ..."
...
echo "[31/48] GET /audit?action=sandbox. finds sandbox lifecycle event ..."
...
echo "[32/48] member user gets 403 from /audit ..."
docker compose exec -T postgres psql -U app -d app -v ON_ERROR_STOP=1 <<SQL >/dev/null
INSERT INTO users (tenant_id, email, password_hash, name, role)
VALUES ((SELECT id FROM tenants WHERE slug='default'),
...
echo "[33/48] GET /metrics with admin JWT returns pca_* metrics ..."
...
echo "[34/48] GET /metrics with static scrape token also works ..."
...
echo "[35/48] GET /metrics without auth is rejected ..."
...
echo "[36/48] vector search ranks semantically similar memories ..."
...
echo "[37/48] keyword mode falls back to ILIKE ..."
...
echo "[38/48] Create dedup returns existing id with created=false ..."
...
echo "[39/48] Distinct content -> new id (no false merge) ..."
...
echo "[40/48] GET /skills lists seeded skills ..."
...
echo "[41/48] GET /skills/:id?include=body returns body ..."
...
echo "[42/48] agent.run with skill_ids=[e2e-marker] injects marker -> 'skill-marker-ok' ..."
...
echo "[43/48] sandbox quota exceeded -> 429 quota_exceeded ..."
...
echo "[44/48] POST /auth/logout revokes bearer token (subsequent requests 401) ..."
...
echo "[45/48] POST /sessions auto-binds sandbox; WS list workspace -> fs.list ..."
...
echo "[46/48] OIDC authorization code flow -> JWT -> GET /me ..."
...
echo "[47/48] memory inject on first session message -> audit memory.inject ..."
...
echo "[48/48] sandbox files list API returns workspace entries ..."
