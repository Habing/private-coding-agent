...
echo "[1/32] starting compose ..."
docker compose up -d --build >/dev/null
sleep 20

echo "[2/32] inserting demo user via psql ..."
...
echo "[8/32] verify 404 after destroy ..."
...
echo "[9/32] chat completion (non-stream) via mock-provider ..."
...
echo "[10/32] chat completion (stream) via mock-provider ..."
...
echo "[11/32] embeddings via mock-provider ..."
...
echo "[12/32] verify model_usage rows ..."
...
echo "[14/32] fs.write + fs.read round-trip ..."
...
echo "[16/32] llm.chat + tool_invocations ..."
...
echo "[17/32] agent.run direct final ..."
...
echo "[18/32] agent.run with tool_call chain ..."
...
echo "[20/32] GET /sessions + GET /sessions/:id/messages ..."
...
echo "[21/32] WS round-trip via docker websocat ..."
...
echo "[22/32] POST /memories x2 (different types) ..."
...
echo "[23/32] GET /memories?type=preference&tag=go filter ..."
...
echo "[24/32] memory.save via tool -> memory.search via tool round-trip ..."
...
echo "[25/32] DELETE /memories/{id} -> GET 404 ..."
...
# ---- Slice 8: Web UI ----
echo "[26/32] GET / returns SPA shell html ..."
...
echo "[27/32] GET /login (SPA fallback) returns the same shell ..."
HTML2=$(curl -fsS http://localhost:8080/login)
echo "$HTML2" | grep -q 'id="root"' || { echo "spa fallback failed for /login"; exit 1; }

echo "[28/32] API not shadowed by SPA fallback: GET /sessions returns JSON ..."
...
# ---- Slice 9: Audit ----
echo "[29/32] GET /audit (admin) returns the access log ..."
...
echo "[30/32] GET /audit?action=auth.login finds login event ..."
...
echo "[31/32] GET /audit?action=sandbox. finds sandbox lifecycle event ..."
