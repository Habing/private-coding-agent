...
echo "[2/64] inserting demo user via psql ..."
...
echo "[8/64] verify 404 after destroy ..."
...
echo "[9/64] chat completion (non-stream) via mock-provider ..."
...
echo "[10/64] chat completion (stream) via mock-provider ..."
...
echo "[11/64] embeddings via mock-provider ..."
...
echo "[12/64] verify model_usage rows ..."
...
echo "[14/64] fs.write + fs.read round-trip ..."
...
echo "[16/64] llm.chat + tool_invocations ..."
...
echo "[17/64] agent.run direct final ..."
...
echo "[18/64] agent.run with tool_call chain ..."
...
echo "[20/64] GET /sessions + GET /sessions/:id/messages ..."
...
echo "[21/64] WS round-trip via docker websocat ..."
...
echo "[21b/64] DELETE /sessions/:id archives session and releases sandbox ..."
curl -fsS -X DELETE "http://localhost:8080/sessions/$SID" \
  -H "Authorization: Bearer $TOK" >/dev/null

echo "[22/64] POST /memories x2 (different types) ..."
...
echo "[23/64] GET /memories?type=preference&tag=go filter ..."
...
echo "[24/64] memory.save via tool -> memory.search via tool round-trip ..."
...
echo "[25/64] DELETE /memories/{id} -> GET 404 ..."
...
# ---- Slice 8: Web UI ----
echo "[26/64] GET / returns SPA shell html ..."
...
echo "[27/64] GET /login (SPA fallback) returns the same shell ..."
HTML2=$(curl -fsS http://localhost:8080/login)
echo "$HTML2" | grep -q 'id="root"' || { echo "spa fallback failed for /login"; exit 1; }

echo "[28/64] API not shadowed by SPA fallback: GET /sessions returns JSON ..."
...
# ---- Slice 9: Audit ----
echo "[29/64] GET /audit (admin) returns the access log ..."
...
echo "[30/64] GET /audit?action=auth.login finds login event ..."
...
echo "[31/64] GET /audit?action=sandbox. finds sandbox lifecycle event ..."
...
echo "[32/64] member user gets 403 from /audit ..."
docker compose exec -T postgres psql -U app -d app -v ON_ERROR_STOP=1 <<SQL >/dev/null
INSERT INTO users (tenant_id, email, password_hash, name, role)
VALUES ((SELECT id FROM tenants WHERE slug='default'),
...
echo "[33/64] GET /metrics with admin JWT returns pca_* metrics ..."
...
echo "[34/64] GET /metrics with static scrape token also works ..."
...
echo "[35/64] GET /metrics without auth is rejected ..."
...
echo "[36/64] vector search ranks semantically similar memories ..."
...
echo "[37/64] keyword mode falls back to ILIKE ..."
...
echo "[38/64] Create dedup returns existing id with created=false ..."
...
echo "[39/64] Distinct content -> new id (no false merge) ..."
...
echo "[40/64] GET /skills lists seeded skills ..."
...
echo "[41/64] GET /skills/:id?include=body returns body ..."
...
echo "[42/64] agent.run with skill_ids=[e2e-marker] injects marker -> 'skill-marker-ok' ..."
...
echo "[43/64] sandbox quota exceeded -> 429 quota_exceeded ..."
...
echo "[44/64] POST /auth/logout revokes bearer token (subsequent requests 401) ..."
...
echo "[45/64] POST /sessions auto-binds sandbox; WS list workspace -> fs.list ..."
...
echo "[46/64] OIDC authorization code flow -> JWT -> GET /me ..."
...
echo "[47/64] memory inject on first session message -> audit memory.inject ..."
...
echo "[48/64] sandbox files list API returns workspace entries ..."
...
echo "[49/64] tenant DB skill (admin CRUD) flows through resolver -> 'tenant-skill-marker-ok' ..."
...
echo "[50/64] sub-agent delegate round-trip + GET /agent/profiles ..."
...
echo "[57/64] workflow CRUD + publish registers workflow.e2e-demo into ToolBus ..."
WF_DSL=$(cat <<'YAML'
id: e2e-demo
...
echo "[58/64] /tools/invoke workflow.e2e-demo records a workflow_runs row ..."
...
echo "[59/64] agent.run with workflow marker fires workflow.e2e-demo tool_call ..."
...
echo "[60/64] dry-run mocks mutating tool nodes ..."
...
echo "[61/64] reflection: archive session -> pending proposal -> admin approve -> memory.search hits ..."
...
echo "[62/64] orchestrator: routing hint injected -> mock acks -> audit logged ..."
...
echo "[63/64] mcp: register mock server -> tools/list -> invoke echo round-trip ..."
...
echo "[64/64] audit hash chain: verify clean -> tamper -> detect -> restore ..."
