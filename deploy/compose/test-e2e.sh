...
echo "[2/62] inserting demo user via psql ..."
...
echo "[8/62] verify 404 after destroy ..."
...
echo "[9/62] chat completion (non-stream) via mock-provider ..."
...
echo "[10/62] chat completion (stream) via mock-provider ..."
...
echo "[11/62] embeddings via mock-provider ..."
...
echo "[12/62] verify model_usage rows ..."
...
echo "[14/62] fs.write + fs.read round-trip ..."
...
echo "[16/62] llm.chat + tool_invocations ..."
...
echo "[17/62] agent.run direct final ..."
...
echo "[18/62] agent.run with tool_call chain ..."
...
echo "[20/62] GET /sessions + GET /sessions/:id/messages ..."
...
echo "[21/62] WS round-trip via docker websocat ..."
...
echo "[21b/62] DELETE /sessions/:id archives session and releases sandbox ..."
curl -fsS -X DELETE "http://localhost:8080/sessions/$SID" \
  -H "Authorization: Bearer $TOK" >/dev/null

echo "[22/62] POST /memories x2 (different types) ..."
...
echo "[23/62] GET /memories?type=preference&tag=go filter ..."
...
echo "[24/62] memory.save via tool -> memory.search via tool round-trip ..."
...
echo "[25/62] DELETE /memories/{id} -> GET 404 ..."
...
# ---- Slice 8: Web UI ----
echo "[26/62] GET / returns SPA shell html ..."
...
echo "[27/62] GET /login (SPA fallback) returns the same shell ..."
HTML2=$(curl -fsS http://localhost:8080/login)
echo "$HTML2" | grep -q 'id="root"' || { echo "spa fallback failed for /login"; exit 1; }

echo "[28/62] API not shadowed by SPA fallback: GET /sessions returns JSON ..."
...
# ---- Slice 9: Audit ----
echo "[29/62] GET /audit (admin) returns the access log ..."
...
echo "[30/62] GET /audit?action=auth.login finds login event ..."
...
echo "[31/62] GET /audit?action=sandbox. finds sandbox lifecycle event ..."
...
echo "[32/62] member user gets 403 from /audit ..."
docker compose exec -T postgres psql -U app -d app -v ON_ERROR_STOP=1 <<SQL >/dev/null
INSERT INTO users (tenant_id, email, password_hash, name, role)
VALUES ((SELECT id FROM tenants WHERE slug='default'),
...
echo "[33/62] GET /metrics with admin JWT returns pca_* metrics ..."
...
echo "[34/62] GET /metrics with static scrape token also works ..."
...
echo "[35/62] GET /metrics without auth is rejected ..."
...
echo "[36/62] vector search ranks semantically similar memories ..."
...
echo "[37/62] keyword mode falls back to ILIKE ..."
...
echo "[38/62] Create dedup returns existing id with created=false ..."
...
echo "[39/62] Distinct content -> new id (no false merge) ..."
...
echo "[40/62] GET /skills lists seeded skills ..."
...
echo "[41/62] GET /skills/:id?include=body returns body ..."
...
echo "[42/62] agent.run with skill_ids=[e2e-marker] injects marker -> 'skill-marker-ok' ..."
...
echo "[43/62] sandbox quota exceeded -> 429 quota_exceeded ..."
...
echo "[44/62] POST /auth/logout revokes bearer token (subsequent requests 401) ..."
...
echo "[45/62] POST /sessions auto-binds sandbox; WS list workspace -> fs.list ..."
...
echo "[46/62] OIDC authorization code flow -> JWT -> GET /me ..."
...
echo "[47/62] memory inject on first session message -> audit memory.inject ..."
...
echo "[48/62] sandbox files list API returns workspace entries ..."
...
echo "[49/62] tenant DB skill (admin CRUD) flows through resolver -> 'tenant-skill-marker-ok' ..."
...
echo "[50/62] sub-agent delegate round-trip + GET /agent/profiles ..."
...
echo "[57/62] workflow CRUD + publish registers workflow.e2e-demo into ToolBus ..."
WF_DSL=$(cat <<'YAML'
id: e2e-demo
...
echo "[58/62] /tools/invoke workflow.e2e-demo records a workflow_runs row ..."
...
echo "[59/62] agent.run with workflow marker fires workflow.e2e-demo tool_call ..."
...
echo "[60/62] dry-run mocks mutating tool nodes ..."
...
echo "[61/62] reflection: archive session -> pending proposal -> admin approve -> memory.search hits ..."
...
echo "[62/62] orchestrator: routing hint injected -> mock acks -> audit logged ..."
