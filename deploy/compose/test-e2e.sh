...
echo "[1/42] starting compose ..."
docker compose up -d --build >/dev/null
sleep 20

echo "[2/42] inserting demo user via psql ..."
...
echo "[8/42] verify 404 after destroy ..."
...
echo "[9/42] chat completion (non-stream) via mock-provider ..."
...
echo "[10/42] chat completion (stream) via mock-provider ..."
...
echo "[11/42] embeddings via mock-provider ..."
...
echo "[12/42] verify model_usage rows ..."
...
echo "[14/42] fs.write + fs.read round-trip ..."
...
echo "[16/42] llm.chat + tool_invocations ..."
...
echo "[17/42] agent.run direct final ..."
...
echo "[18/42] agent.run with tool_call chain ..."
...
echo "[20/42] GET /sessions + GET /sessions/:id/messages ..."
...
echo "[21/42] WS round-trip via docker websocat ..."
...
# Client may disconnect after a few frames; server still completes the run
# (session WS uses context.WithoutCancel for SendMessage).
...
echo "[22/42] POST /memories x2 (different types) ..."
...
echo "[23/42] GET /memories?type=preference&tag=go filter ..."
...
echo "[24/42] memory.save via tool -> memory.search via tool round-trip ..."
...
echo "[25/42] DELETE /memories/{id} -> GET 404 ..."
...
# ---- Slice 8: Web UI ----
echo "[26/42] GET / returns SPA shell html ..."
...
echo "[27/42] GET /login (SPA fallback) returns the same shell ..."
HTML2=$(curl -fsS http://localhost:8080/login)
echo "$HTML2" | grep -q 'id="root"' || { echo "spa fallback failed for /login"; exit 1; }

echo "[28/42] API not shadowed by SPA fallback: GET /sessions returns JSON ..."
...
# ---- Slice 9: Audit ----
echo "[29/42] GET /audit (admin) returns the access log ..."
...
echo "[30/42] GET /audit?action=auth.login finds login event ..."
...
echo "[31/42] GET /audit?action=sandbox. finds sandbox lifecycle event ..."
...
echo "[32/42] member user gets 403 from /audit ..."
docker compose exec -T postgres psql -U app -d app -v ON_ERROR_STOP=1 <<SQL >/dev/null
INSERT INTO users (tenant_id, email, password_hash, name, role)
VALUES ((SELECT id FROM tenants WHERE slug='default'),
...
echo "[33/42] GET /metrics with admin JWT returns pca_* metrics ..."
...
echo "[34/42] GET /metrics with static scrape token also works ..."
...
echo "[35/42] GET /metrics without auth is rejected ..."
...
echo "[36/42] vector search ranks semantically similar memories ..."
...
echo "[37/42] keyword mode falls back to ILIKE ..."
...
echo "[38/42] Create dedup returns existing id with created=false ..."
...
echo "[39/42] Distinct content -> new id (no false merge) ..."
...
echo "[40/42] GET /skills lists seeded skills ..."
...
echo "[41/42] GET /skills/:id?include=body returns body ..."
...
echo "[42/42] agent.run with skill_ids=[e2e-marker] injects marker -> 'skill-marker-ok' ..."
